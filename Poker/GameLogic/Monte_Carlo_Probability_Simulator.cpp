...
std::vector<float> Winning_Probability (Table table, std::vector<Card> hand, int num_simulations, int num_players= 0) { //num_players is the number of other players
    ...
    for (int i = 0; i < num_simulations; i++) { // we iterate through the number of simulations
        ...
        for (int j=0; j<num_players; j++) { //for each simulation, we construct the hand of the other players
            ...
            for (int k=0; k<2; k++) { //for each player, we iterate through the 2 cards in his hand and construct both
                ...
                if(deck.getCurrentCard() != hand[0] && deck.getCurrentCard()!= hand[1]) { // we ignore the cards that the first player already has when dealing cards
