...
int BotPu::cnt5 = 0, BotPu::cnt4 = 0, BotPu::cnt_hand = 0;
...
int BotPu::get_cfr4(int player_num, int handid, int riverid) {
    ...
    if(cfr4_norm[0] == 0 && cfr4_norm[1] == 0 && cfr4_norm[2] == 0)
        return 0;
    float choice_dist[3];
    for(int i = 0; i < 3; i++)
...
void BotPu::Action(){
    ...
    if(round == 4) {
        ...
        if(player_action2[0] == 2) {
            ...
            if(win0 == false) {
                ...
                gain[0] = (cnt_raised1 * cnt_notfolded1 + cnt_raised2 * cnt_notfolded2 + tableInfo.pot) / cnt_winner
                          - (tableInfo.playerInfo[tableInfo.current_player].bet / tableInfo.BBValue) - cnt_raised1 - cnt_raised2;
                gain[1] = ((cnt_raised1 + 1) * cnt_notfolded1 + cnt_raised2 * cnt_notfolded2 + tableInfo.pot) / cnt_winner
    ...
    if(round == 5) {
        ...
        for(int _ = 0; _ < iter_lim; _++) {
            ...
            if(win0) {
                gain[0] = (cnt_raised * cnt_notfolded + tableInfo.pot) / cnt_winner - (tableInfo.playerInfo[tableInfo.current_player].bet / tableInfo.BBValue) - cnt_raised;
                gain[1] = ((cnt_raised + 1) * cnt_notfolded + tableInfo.pot) / cnt_winner - (tableInfo.playerInfo[tableInfo.current_player].bet / tableInfo.BBValue) - cnt_raised - 1;
            }
            else {
